...
import type { PortfolioPage, PageComponent } from '../types';
...
export default function ComponentBuilder() {
  ...
  const [selectedComp, setSelectedComp] = useState<PageComponent | null>(null);
  ...
  const handleDelete = async (compId: string) => {
    ...
    try { await adminApi.deleteComponent(selectedPageId, compId); await fetchComponents(); if (selectedComp?._id === compId) setSelectedComp(null); }
    catch (err) { console.error('Failed to delete', err); }
  };

  const handleDuplicate = async (compId: string) => {
    try { await adminApi.duplicateComponent(selectedPageId, compId); await fetchComponents(); }
  ...
  const handleUpdateComponent = async (field: string, value: unknown) => {
            ...
            {components.map((comp, idx) => (
              ...
              >
                ...
                <div className="page-builder-item-actions">
                  ...
                  <button className="btn btn-ghost btn-sm" style={{ color: 'var(--danger)' }} onClick={(e) => { e.stopPropagation(); handleDelete(comp._id); }} data-tooltip="Delete">
